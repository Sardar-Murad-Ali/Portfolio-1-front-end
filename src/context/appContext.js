...
const AppProvider = ({ children }) => {
  ...
  const uploadImageForRegister=async (event)=>{
    ...
    //  const {data:{image:{src}}} = await axios.post("/api/v1/post/upload"
     ...
     const {data:{image:{src}}} = await axios.post("api/v1/auth/registerUploadImage"
     ,formData,{
      headers:{
  ...
  function changepage(page){
   ...
   const createMessage=async ({message})=>{
    ...
    try {
      await axios.post("api/v1/message",{message},{headers:{Authorization:`Bearer ${token}`}})
    } catch (error) {
     dispatch({type:CREATE_MESSAGE_ERROR,payload:{data:error.response.data.msg}})
    }
     clearAlert()
   } 
